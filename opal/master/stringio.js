...
(function($opal) {
  ...
  return (function($base, $super) {
    ...
    self.$include((($scope.get('IO'))._scope.get('Readable')));

    self.$include((($scope.get('IO'))._scope.get('Writable')));
    ...
    def.$seek = function(pos, whence) {
      ...
      if (whence == null) {
        whence = (($scope.get('IO'))._scope.get('SEEK_SET'))
      }
      $case = whence;if ((($scope.get('IO'))._scope.get('SEEK_SET'))['$===']($case)) {if (pos['$>='](0)) {
        } else {
        self.$raise((($scope.get('Errno'))._scope.get('EINVAL')))
      };
      self.position = pos;}else if ((($scope.get('IO'))._scope.get('SEEK_CUR'))['$===']($case)) {if (self.position['$+'](pos)['$>'](self.string.$length())) {
        self.position = self.string.$length()
        } else {
        self.position = self.position['$+'](pos)
      }}else if ((($scope.get('IO'))._scope.get('SEEK_END'))['$===']($case)) {if (pos['$>'](self.string.$length())) {
        self.position = 0
        } else {
        self.position = self.position['$-'](pos)
      }};
